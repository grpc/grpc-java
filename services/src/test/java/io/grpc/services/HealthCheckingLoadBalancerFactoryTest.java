...
public class HealthCheckingLoadBalancerFactoryTest {
  ...
  private final FakeHelper origHelperRef = new FakeHelper();
  private final Helper origHelper = mock(Helper.class, delegatesTo(origHelperRef));
  ...
  public void typicalWorkflow() {
    ...
    for (int i = NUM_SUBCHANNELS - 1; i >= 0; i--) {
      ...
      assertThat(subchannel.logger.logs).isEmpty();
      ...
      assertThat(subchannel.logger.logs).containsExactly(
          "INFO: CONNECTING: Starting health-check for \"FooService\"");
      subchannel.logger.logs.clear();
  ...
  public void healthCheckDisabledWhenServiceNotImplemented() {
    ...
    subchannels[0].logger.logs.clear();
    ...
    assertThat(subchannels[0].logger.logs).containsExactly(
  ...
  @Test
  public void serviceConfigHasMalformedServiceName() {
    HashMap<String, Object> serviceConfig = new HashMap<>();
    serviceConfig.put("healthCheckConfig", "I thought I was a valid hc config");
    Attributes resolutionAttrs = Attributes.newBuilder()
        .set(GrpcAttributes.NAME_RESOLVER_SERVICE_CONFIG, serviceConfig).build();
    hcLbEventDelivery.handleResolvedAddressGroups(resolvedAddressList, resolutionAttrs);
    verify(origLb).handleResolvedAddressGroups(same(resolvedAddressList), same(resolutionAttrs));

    assertThat(origHelperRef.logger.logs)
        .containsExactly("WARNING: Malformed config for health-check");
  }
  ...
  private class FakeSubchannel extends Subchannel {
    ...
    final FakeChannelLogger logger = new FakeChannelLogger();
  ...
  private static class FakeChannelLogger extends ChannelLogger {
