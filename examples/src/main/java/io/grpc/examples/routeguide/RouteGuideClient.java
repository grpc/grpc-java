...
public class RouteGuideClient {
  ...
  public void listFeaturesSync(int lowLat, int lowLon, int hiLat, int hiLon) {
    ...
    if (call.getStatus().isOk()) {
      warning("RPC failed: {0}", call.getStatus());
      if (testHelper != null) {
        testHelper.onRpcError(call.getStatus().asRuntimeException());
      }
    }
  }
   ...
   * server.
   */
  public void recordRouteSync(List<Feature> features, int numPoints) throws InterruptedException {
    info("*** RecordRoute");

    SyncClientCall<Point, RouteSummary> call =
        SyncCall.call(
            asyncStub.getChannel(),
