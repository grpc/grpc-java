...
public class PreSerializedServer {
  ...
  static class GreeterImpl implements GreeterGrpc.AsyncService, BindableService {
    ...
    private static <ReqT, RespT> ServerServiceDefinition replaceMethod(
        ...
        MethodDescriptor<ReqT, RespT> desc,
      ...
      ServiceDescriptor srvDesc = def.getServiceDescriptor();
      ServiceDescriptor.Builder srvBuilder = ServiceDescriptor.newBuilder(srvDesc.getName())
          .setSchemaDescriptor(srvDesc.getSchemaDescriptor())
          .addMethod(desc); // Add the modified method
      // Copy methods other than the modified one
      for (MethodDescriptor<?,?> md : srvDesc.getMethods()) {
        if (desc.getFullMethodName().equals(md.getFullMethodName())) {
          continue;
        }
        srvBuilder.addMethod(md);
      }
      ServerServiceDefinition.Builder b = ServerServiceDefinition.builder(srvBuilder.build())
          .addMethod(desc, handler); // Add the modified method
      // Copy methods other than the modified one
      for (ServerMethodDefinition<?,?> smd : def.getMethods()) {
        if (desc.getFullMethodName().equals(smd.getMethodDescriptor().getFullMethodName())) {
          continue;
        }
        b.addMethod(smd);
      }
