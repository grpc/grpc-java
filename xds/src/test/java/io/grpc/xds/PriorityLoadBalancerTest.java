...
public class PriorityLoadBalancerTest {
  ...
  @Test
  public void priorityPicker_prependsNumericPriority_notPriorityName() throws Exception {
    // The priority name is generated by CdsLoadBalancer2 from the xDS cluster name, so it is
    // operator controlled and unbounded. It must not end up in the delay type, which is used as a
    // metric label.
    String priorityName = "my-cluster[child0]";
    PriorityChildConfig priorityChildConfig0 =
        new PriorityChildConfig(newChildConfig(fooLbProvider, new Object()), true);
    PriorityLbConfig priorityLbConfig = new PriorityLbConfig(
        ImmutableMap.of(priorityName, priorityChildConfig0), ImmutableList.of(priorityName));
    ...
    Helper helper0 = Iterables.getOnlyElement(fooHelpers);  // priority index 0
    helper0.updateBalancingState(CONNECTING, fixedNoResultPicker("connecting", "child_reason"));

    PickResult result = latestPickResult(CONNECTING);
    assertThat(result.getDelayType()).isEqualTo("0:connecting");
    assertThat(result.getDelayType()).doesNotContain("my-cluster");
    // The reason is high-cardinality by design and keeps the priority name, so that operators can
    // still identify the cluster from a trace.
    assertThat(result.getDelayReason())
        .isEqualTo("waiting on priority 0 (child 'my-cluster[child0]'): child_reason");
  }
  ...
  public void priorityPicker_nestedPriorities_composesNumericPrefixes() throws Exception {
    ...
    Helper helper0 = Iterables.getOnlyElement(fooHelpers);  // priority index 0
    // Simulates a nested priority policy that has already prefixed its own numeric priority.
    helper0.updateBalancingState(CONNECTING, fixedNoResultPicker(
        "1:connecting", "waiting on priority 1 (child 'inner'): child_reason"));

    PickResult result = latestPickResult(CONNECTING);
    assertThat(result.getDelayType()).isEqualTo("0:1:connecting");
    assertThat(result.getDelayReason()).isEqualTo(
        "waiting on priority 0 (child 'p0'): "
            + "waiting on priority 1 (child 'inner'): child_reason");
  }

  @Test
  public void priorityPicker_usesIndexOfPriorityInList() throws Exception {
    PriorityChildConfig priorityChildConfig0 =
        new PriorityChildConfig(newChildConfig(fooLbProvider, new Object()), true);
    PriorityChildConfig priorityChildConfig1 =
        new PriorityChildConfig(newChildConfig(fooLbProvider, new Object()), true);
    PriorityLbConfig priorityLbConfig = new PriorityLbConfig(
        ImmutableMap.of("cluster[child0]", priorityChildConfig0,
            "cluster[child1]", priorityChildConfig1),
        ImmutableList.of("cluster[child0]", "cluster[child1]"));
    ...
    // Fail over from priority 0 to priority 1.
    Helper helper0 = Iterables.getOnlyElement(fooHelpers);
    helper0.updateBalancingState(
        TRANSIENT_FAILURE, new FixedResultPicker(PickResult.withError(Status.UNAVAILABLE)));
    assertThat(fooHelpers).hasSize(2);
    Helper helper1 = Iterables.getLast(fooHelpers);
    helper1.updateBalancingState(CONNECTING, fixedNoResultPicker("connecting", "child_reason"));

    PickResult result = latestPickResult(CONNECTING);
    assertThat(result.getDelayType()).isEqualTo("1:connecting");
    assertThat(result.getDelayReason())
        .isEqualTo("waiting on priority 1 (child 'cluster[child1]'): child_reason");
  }

  @Test
  public void priorityPicker_prefixFollowsPriorityListChanges() throws Exception {
    PriorityChildConfig priorityChildConfig0 =
        new PriorityChildConfig(newChildConfig(fooLbProvider, new Object()), true);
    PriorityChildConfig priorityChildConfig1 =
        new PriorityChildConfig(newChildConfig(fooLbProvider, new Object()), true);
    ...
    Helper helper1 = Iterables.getLast(fooHelpers);
    helper1.updateBalancingState(CONNECTING, fixedNoResultPicker("connecting", "child_reason"));
    assertThat(latestPickResult(CONNECTING).getDelayType()).isEqualTo("1:connecting");

    // p0 is dropped from the config, so p1 becomes priority 0 and the delay type must follow.
    priorityLbConfig = new PriorityLbConfig(
        ImmutableMap.of("p1", priorityChildConfig1), ImmutableList.of("p1"));
    priorityLb.acceptResolvedAddresses(
        ResolvedAddresses.newBuilder()
            .setAddresses(ImmutableList.<EquivalentAddressGroup>of())
            .setLoadBalancingPolicyConfig(priorityLbConfig)
    ...
    assertThat(result.getDelayType()).isEqualTo("0:connecting");
    assertThat(result.getDelayReason())
        .isEqualTo("waiting on priority 0 (child 'p1'): child_reason");
  }
  ...
  public void initialChildPicker_returnsAnnotatedDelayAttributes() throws Exception {
    ...
    // Already carries the numeric prefix, so the delay type does not change (which would end one
    // delay and start another) once the child reports its first picker.
    assertThat(result.getDelayType()).isEqualTo("0:connecting");
    assertThat(result.getDelayReason()).isEqualTo(
  ...
  private static SubchannelPicker fixedNoResultPicker(String delayType, String delayReason) {
    return new FixedResultPicker(PickResult.withNoResult(delayType, delayReason));
  }

  /** Returns the pick result of the most recent picker reported for {@code state}. */
  private PickResult latestPickResult(ConnectivityState state) {
    verify(helper, atLeastOnce()).updateBalancingState(eq(state), pickerCaptor.capture());
    return pickerCaptor.getValue().pickSubchannel(mock(PickSubchannelArgs.class));
  }
