...
/**
 * Tests for {@link XdsClientImpl} with xDS-v3 protocol (but resources in response is still v2).
 */
...
public class XdsClientImplTest {
  ...
  public void setUp() throws IOException {
    ...
    xdsClient =
        new XdsClientImpl(
            ...
            servers,
            channelFactory,
            EnvoyProtoData.Node.fromEnvoyProtoNode(NODE),
            syncContext,
  ...
  public void ldsResponseWithoutMatchingResource() {
    ...
    verify(requestObserver).onNext(eq(
        buildDiscoveryRequest(NODE, "", TARGET_AUTHORITY, EnvoyProtoData.ADS_TYPE_URL_LDS, "")));
    ...
    DiscoveryResponse response =
        buildDiscoveryResponse("0", listeners, EnvoyProtoData.ADS_TYPE_URL_LDS_V2, "0000");
    responseObserver.onNext(response);

    // Client sends an ACK LDS request.
    verify(requestObserver).onNext(eq(
        buildDiscoveryRequest(
            NODE, "0", TARGET_AUTHORITY, EnvoyProtoData.ADS_TYPE_URL_LDS, "0000")));
  ...
  public void failToFindVirtualHostInLdsResponseInLineRouteConfig() {
    ...
    verify(requestObserver).onNext(eq(
        buildDiscoveryRequest(
            NODE, "", TARGET_AUTHORITY, EnvoyProtoData.ADS_TYPE_URL_LDS, "")));
    assertThat(fakeClock.getPendingTasks(LDS_RESOURCE_FETCH_TIMEOUT_TASK_FILTER)).hasSize(1);
    ...
    verify(requestObserver)
        .onNext(
            argThat(new DiscoveryRequestMatcher("", TARGET_AUTHORITY,
                EnvoyProtoData.ADS_TYPE_URL_LDS, "0000")));
  ...
  public void rdsResponseWithoutMatchingResource() {
    ...
    verify(requestObserver).onNext(eq(
        buildDiscoveryRequest(
            NODE, "", "route-foo.googleapis.com", EnvoyProtoData.ADS_TYPE_URL_RDS, "")));
    ...
    response = buildDiscoveryResponse("0", routeConfigs, EnvoyProtoData.ADS_TYPE_URL_RDS_V2,
        "0000");
    responseObserver.onNext(response);
    ...
    verify(requestObserver).onNext(eq(
        buildDiscoveryRequest(
            NODE, "0", "route-foo.googleapis.com", EnvoyProtoData.ADS_TYPE_URL_RDS, "0000")));
    ...
    verify(requestObserver)
        .onNext(
            argThat(new DiscoveryRequestMatcher("", "route-foo.googleapis.com",
                EnvoyProtoData.ADS_TYPE_URL_RDS, "0000")));
  ...
  public void notifyUpdatedResources() {
    ...
    response =
        buildDiscoveryResponse("1", listeners, EnvoyProtoData.ADS_TYPE_URL_LDS_V2, "0001");
    responseObserver.onNext(response);

    // Client sends an ACK LDS request.
    verify(requestObserver).onNext(eq(
        buildDiscoveryRequest(
            NODE, "1", TARGET_AUTHORITY, EnvoyProtoData.ADS_TYPE_URL_LDS, "0001")));
    ...
    response =
        buildDiscoveryResponse("2", listeners, EnvoyProtoData.ADS_TYPE_URL_LDS_V2, "0002");
    responseObserver.onNext(response);

    // Client sends an ACK LDS request.
    verify(requestObserver).onNext(eq(
        buildDiscoveryRequest(
            NODE, "2", TARGET_AUTHORITY, EnvoyProtoData.ADS_TYPE_URL_LDS, "0002")));

    // Client sends an (first) RDS request.
    verify(requestObserver).onNext(eq(
        buildDiscoveryRequest(
            NODE, "", "some-route-to-foo.googleapis.com", EnvoyProtoData.ADS_TYPE_URL_RDS, "")));
    ...
    verify(requestObserver).onNext(eq(
        buildDiscoveryRequest(
            NODE, "0", "some-route-to-foo.googleapis.com", EnvoyProtoData.ADS_TYPE_URL_RDS,
            "0000")));
    ...
    response = buildDiscoveryResponse("1", routeConfigs, EnvoyProtoData.ADS_TYPE_URL_RDS_V2,
        "0001");
    responseObserver.onNext(response);
    ...
    verify(requestObserver).onNext(eq(
        buildDiscoveryRequest(
            NODE, "1", "some-route-to-foo.googleapis.com", EnvoyProtoData.ADS_TYPE_URL_RDS,
            "0001")));
  ...
  public void waitRdsResponsesForRequestedResource() {
    ...
    verify(requestObserver).onNext(eq(
        buildDiscoveryRequest(
            NODE, "1", "route-foo.googleapis.com", EnvoyProtoData.ADS_TYPE_URL_RDS, "0001")));
  ...
  public void updateRdsRequestResourceWhileInitialResourceFetchInProgress() {
    ...
    response = buildDiscoveryResponse("1", listeners, EnvoyProtoData.ADS_TYPE_URL_LDS_V2, "0001");
    responseObserver.onNext(response);

    // Client sent a new RDS request with updated resource name.
    verify(requestObserver).onNext(eq(
        buildDiscoveryRequest(
            NODE, "", "route-bar.googleapis.com", EnvoyProtoData.ADS_TYPE_URL_RDS, "")));
  ...
  public void cdsResponseWithoutMatchingResource() {
    ...
    verify(requestObserver).onNext(eq(
        buildDiscoveryRequest(
            NODE, "", "cluster-foo.googleapis.com", EnvoyProtoData.ADS_TYPE_URL_CDS, "")));
    ...
    DiscoveryResponse response =
        buildDiscoveryResponse("0", clusters, EnvoyProtoData.ADS_TYPE_URL_CDS_V2, "0000");
    responseObserver.onNext(response);

    // Client sent an ACK CDS request.
    verify(requestObserver).onNext(eq(
        buildDiscoveryRequest(
            NODE, "0", "cluster-foo.googleapis.com", EnvoyProtoData.ADS_TYPE_URL_CDS, "0000")));
  ...
  public void cdsResponseWithMatchingResource() {
    ...
    clusters = ImmutableList.of(
        ...
        Any.pack(buildCluster("cluster-baz.googleapis.com", null, false)));
    response =
        buildDiscoveryResponse("1", clusters, EnvoyProtoData.ADS_TYPE_URL_CDS_V2, "0001");
    responseObserver.onNext(response);

    // Client sent an ACK CDS request.
    verify(requestObserver).onNext(eq(
        buildDiscoveryRequest(
            NODE, "1", "cluster-foo.googleapis.com", EnvoyProtoData.ADS_TYPE_URL_CDS, "0001")));
        ...
        .onNext(
            argThat(
                new DiscoveryRequestMatcher("",
                    ...
                    EnvoyProtoData.ADS_TYPE_URL_CDS, "")));
        ...
        .onNext(
            argThat(
                new DiscoveryRequestMatcher("0",
                    ImmutableList.of("cluster-foo.googleapis.com", "cluster-bar.googleapis.com"),
                    EnvoyProtoData.ADS_TYPE_URL_CDS, "0000")));
        ...
        .onNext(
            argThat(
                new DiscoveryRequestMatcher("1",
                    ImmutableList.of("cluster-foo.googleapis.com", "cluster-bar.googleapis.com"),
                    EnvoyProtoData.ADS_TYPE_URL_CDS, "0001")));
  ...
  public void addRemoveClusterWatchers() {
    ...
    verify(requestObserver).onNext(eq(
        buildDiscoveryRequest(
            NODE, "1", "cluster-bar.googleapis.com", EnvoyProtoData.ADS_TYPE_URL_CDS, "0001")));
    ...
    response =
        buildDiscoveryResponse("2", clusters, EnvoyProtoData.ADS_TYPE_URL_CDS_V2, "0002");
    responseObserver.onNext(response);
        ...
        .onNext(
            argThat(
                new DiscoveryRequestMatcher("2", ImmutableList.<String>of(),
                    EnvoyProtoData.ADS_TYPE_URL_CDS, "0002")));
    ...
    verify(requestObserver).onNext(eq(
        buildDiscoveryRequest(
            NODE, "2", "cluster-foo.googleapis.com", EnvoyProtoData.ADS_TYPE_URL_CDS, "0002")));
    ...
    clusters = ImmutableList.of(
        ...
        Any.pack(
            buildCluster("cluster-bar.googleapis.com", null, false)));
    response =
        buildDiscoveryResponse("3", clusters, EnvoyProtoData.ADS_TYPE_URL_CDS_V2, "0003");
    responseObserver.onNext(response);
    ...
    verify(requestObserver).onNext(eq(
        buildDiscoveryRequest(
            NODE, "3", "cluster-foo.googleapis.com", EnvoyProtoData.ADS_TYPE_URL_CDS, "0003")));
  }
  ...
  public void edsResponseWithoutMatchingResource() {
    ...
    verify(requestObserver).onNext(eq(
        buildDiscoveryRequest(
            NODE, "", "cluster-foo.googleapis.com", EnvoyProtoData.ADS_TYPE_URL_EDS, "")));
    ...
    verify(requestObserver).onNext(eq(
        buildDiscoveryRequest(
            NODE, "0", "cluster-foo.googleapis.com", EnvoyProtoData.ADS_TYPE_URL_EDS, "0000")));
  ...
  public void edsResponseWithMatchingResource() {
    ...
    verify(requestObserver).onNext(eq(
        buildDiscoveryRequest(
            NODE, "1", "cluster-foo.googleapis.com", EnvoyProtoData.ADS_TYPE_URL_EDS, "0001")));
    ...
    verify(requestObserver)
        .onNext(
            argThat(
                new DiscoveryRequestMatcher("",
                    ImmutableList.of("cluster-foo.googleapis.com", "cluster-bar.googleapis.com"),
                    EnvoyProtoData.ADS_TYPE_URL_EDS, "")));
    ...
    verify(requestObserver)
        .onNext(
            argThat(
                new DiscoveryRequestMatcher("0",
                    ImmutableList.of("cluster-foo.googleapis.com", "cluster-bar.googleapis.com"),
                    EnvoyProtoData.ADS_TYPE_URL_EDS, "0000")));
    ...
    verify(requestObserver)
        .onNext(
            argThat(
                new DiscoveryRequestMatcher("1",
                    ImmutableList.of("cluster-foo.googleapis.com", "cluster-bar.googleapis.com"),
                    EnvoyProtoData.ADS_TYPE_URL_EDS, "0001")));
  ...
  public void addRemoveEndpointWatchers() {
    ...
    verify(requestObserver).onNext(eq(
        buildDiscoveryRequest(
            NODE, "1", "cluster-bar.googleapis.com", EnvoyProtoData.ADS_TYPE_URL_EDS, "0001")));
        ...
        .onNext(
            argThat(
                new DiscoveryRequestMatcher("2",
                    ImmutableList.<String>of(),  // empty resources
                    EnvoyProtoData.ADS_TYPE_URL_EDS, "0002")));
    ...
    verify(requestObserver).onNext(eq(
        buildDiscoveryRequest(
            NODE, "2", "cluster-bar.googleapis.com", EnvoyProtoData.ADS_TYPE_URL_EDS, "0002")));
        ...
        .onNext(
            argThat(
                new DiscoveryRequestMatcher("3",
                    ...
                    EnvoyProtoData.ADS_TYPE_URL_EDS, "0003")));
  }
  ...
  public void streamClosedAndRetryWhenResolvingConfig() {
    ...
    DiscoveryResponse rdsResponse =
        buildDiscoveryResponse("0", routeConfigs, EnvoyProtoData.ADS_TYPE_URL_RDS_V2, "0000");
  ...
  public void streamClosedAndRetry() {
    ...
    verify(requestObserver).onNext(eq(
        buildDiscoveryRequest(
            NODE, "", "cluster.googleapis.com", EnvoyProtoData.ADS_TYPE_URL_CDS, "")));
    ...
    verify(requestObserver).onNext(eq(
        buildDiscoveryRequest(
            NODE, "", "cluster.googleapis.com", EnvoyProtoData.ADS_TYPE_URL_EDS, "")));
  ...
  public void streamClosedAndRetryRaceWithAddingAndRemovingWatchers() {
    ...
    verify(requestObserver).onNext(eq(
        buildDiscoveryRequest(
            NODE, "", ImmutableList.<String>of(), EnvoyProtoData.ADS_TYPE_URL_EDS, "")));
    ...
    verify(requestObserver).onNext(eq(
        buildDiscoveryRequest(
            NODE, "", "cluster2.googleapis.com", EnvoyProtoData.ADS_TYPE_URL_EDS, "")));
    ...
    verify(requestObserver).onNext(eq(
        buildDiscoveryRequest(NODE, "", TARGET_AUTHORITY, EnvoyProtoData.ADS_TYPE_URL_LDS, "")));
    verify(requestObserver, never()).onNext(eq(
        buildDiscoveryRequest(
            NODE, "", "cluster.googleapis.com", EnvoyProtoData.ADS_TYPE_URL_CDS, "")));
    verify(requestObserver, never()).onNext(eq(
        buildDiscoveryRequest(
                ...
                buildRouteConfiguration(
                    ...
                    ImmutableList.of(
                        buildVirtualHost(
                            ...
                            "cluster.googleapis.com")))));
    DiscoveryResponse response =
        buildDiscoveryResponse("213", routeConfigs, EnvoyProtoData.ADS_TYPE_URL_RDS_V2, "0052");
  ...
  public void messagePrinter_printCdsResponse() {
    ...
    List<Any> clusters = ImmutableList.of(
        ...
        Any.pack(buildCluster("cluster-foo.googleapis.com", null, false)));
    DiscoveryResponse response =
        buildDiscoveryResponse("14", clusters, EnvoyProtoData.ADS_TYPE_URL_CDS_V2, "8");
