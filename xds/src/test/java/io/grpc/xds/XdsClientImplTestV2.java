...
/**
 * Tests for {@link XdsClientImpl} with xDS-v2 protocol.
 */
@RunWith(JUnit4.class)
public class XdsClientImplTestV2 {
   ...
   * The LDS response is ACKed. The config watcher is notified with resource unavailable after its
  ...
  public void ldsResponseWithoutMatchingResource() {
    ...
    verify(requestObserver)
        .onNext(eq(buildDiscoveryRequestV2(NODE, "", TARGET_AUTHORITY,
            EnvoyProtoData.ADS_TYPE_URL_LDS_V2, "")));
    ...
    verify(requestObserver)
        .onNext(eq(buildDiscoveryRequestV2(NODE, "0", TARGET_AUTHORITY,
            EnvoyProtoData.ADS_TYPE_URL_LDS_V2, "0000")));
   ...
   * VirtualHost with domains matching the requested hostname. The LDS response is NACKed, as if the
   * XdsClient has not received this response. The config watcher is notified with an error after
   * its response timer expires..
   */
  @Test
  ...
  /**
   * Client resolves the virtual host config from an LDS response that contains a RouteConfiguration
   * message directly in-line for the requested resource. No RDS is needed. The LDS response is
   * ACKed. The config watcher is notified with an update.
   */
  @Test
   ...
   * The RDS response is ACKed. After the resource fetch timeout expires, watcher waiting for the
   * resource is notified with resource unavailable.
   */
  @Test
  public void rdsResponseWithoutMatchingResource() {
    ...
    verify(requestObserver)
        .onNext(eq(buildDiscoveryRequestV2(NODE, "", "route-foo.googleapis.com",
            EnvoyProtoData.ADS_TYPE_URL_RDS_V2, "")));
    ...
    response = buildDiscoveryResponseV2("0", routeConfigs, EnvoyProtoData.ADS_TYPE_URL_RDS_V2,
        "0000");
    responseObserver.onNext(response);

    // Client sends an ACK RDS request.
    verify(requestObserver)
        .onNext(eq(buildDiscoveryRequestV2(NODE, "0", "route-foo.googleapis.com",
            EnvoyProtoData.ADS_TYPE_URL_RDS_V2, "0000")));
   ...
   * RDS response is ACKed. The config watcher is notified with an update.
   */
  @Test
   ...
   * requested resource. The RDS response is ACKed. The config watcher is notified with an update.
   */
  @Test
  public void resolveVirtualHostWithPathMatchingInRdsResponse() {
    ...
    verify(requestObserver)
        .onNext(eq(
            buildDiscoveryRequestV2(NODE, "0", "route-foo.googleapis.com",
                EnvoyProtoData.ADS_TYPE_URL_RDS_V2, "0000")));
   ...
   * RouteConfiguration message for the requested resource. But the RouteConfiguration message is
   * invalid as it does not contain any VirtualHost with domains matching the requested hostname.
   ...
   * Client receives an RDS response (after a previous LDS request-response) containing a
   * RouteConfiguration message for the requested resource. But the RouteConfiguration message is
   * invalid as the VirtualHost with domains matching the requested hostname contains invalid data,
   * its RouteAction message is absent. The RDS response is NACKed, as if the XdsClient has not
  ...
  public void notifyUpdatedResources() {
    ...
    response =
        buildDiscoveryResponseV2("1", listeners, EnvoyProtoData.ADS_TYPE_URL_LDS_V2, "0001");
    responseObserver.onNext(response);

    // Client sends an ACK LDS request.
    verify(requestObserver)
        .onNext(eq(buildDiscoveryRequestV2(NODE, "1", TARGET_AUTHORITY,
            EnvoyProtoData.ADS_TYPE_URL_LDS_V2, "0001")));
    ...
    response =
        buildDiscoveryResponseV2("2", listeners, EnvoyProtoData.ADS_TYPE_URL_LDS_V2, "0002");
    responseObserver.onNext(response);

    // Client sends an ACK LDS request.
    verify(requestObserver)
        .onNext(eq(buildDiscoveryRequestV2(NODE, "2", TARGET_AUTHORITY,
            EnvoyProtoData.ADS_TYPE_URL_LDS_V2, "0002")));

    // Client sends an (first) RDS request.
    verify(requestObserver)
        .onNext(eq(
            buildDiscoveryRequestV2(NODE, "", "some-route-to-foo.googleapis.com",
                EnvoyProtoData.ADS_TYPE_URL_RDS_V2, "")));
    ...
    // Client sent an ACK RDS request.
    verify(requestObserver)
        .onNext(eq(XdsClientTestHelper
            .buildDiscoveryRequestV2(NODE, "0", "some-route-to-foo.googleapis.com",
                EnvoyProtoData.ADS_TYPE_URL_RDS_V2, "0000")));
    ...
    response = buildDiscoveryResponseV2("1", routeConfigs, EnvoyProtoData.ADS_TYPE_URL_RDS_V2,
        "0001");
    responseObserver.onNext(response);
    ...
    verify(requestObserver)
        .onNext(eq(XdsClientTestHelper
            .buildDiscoveryRequestV2(NODE, "1", "some-route-to-foo.googleapis.com",
                EnvoyProtoData.ADS_TYPE_URL_RDS_V2, "0001")));
    ...
    response =
        buildDiscoveryResponseV2("3", ImmutableList.<Any>of(),
            EnvoyProtoData.ADS_TYPE_URL_LDS_V2, "0003");
    responseObserver.onNext(response);
  ...
  /**
   * Client receives multiple RDS responses without RouteConfiguration for the requested resource.
  ...
  public void waitRdsResponsesForRequestedResource() {
    ...
    // Client sent an ACK RDS request.
    verify(requestObserver)
        .onNext(eq(buildDiscoveryRequestV2(NODE, "1", "route-foo.googleapis.com",
            EnvoyProtoData.ADS_TYPE_URL_RDS_V2, "0001")));
  ...
  public void routeConfigurationRemovedNotifiedToWatcher() {
    ...
    response =
        buildDiscoveryResponseV2("1", ImmutableList.<Any>of(), // empty
            EnvoyProtoData.ADS_TYPE_URL_LDS_V2, "0001");
    responseObserver.onNext(response);
  ...
  public void updateRdsRequestResourceWhileInitialResourceFetchInProgress() {
    ...
    response = buildDiscoveryResponseV2("1", listeners, EnvoyProtoData.ADS_TYPE_URL_LDS_V2, "0001");
    responseObserver.onNext(response);

    // Client sent a new RDS request with updated resource name.
    verify(requestObserver)
        .onNext(eq(buildDiscoveryRequestV2(NODE, "", "route-bar.googleapis.com",
            EnvoyProtoData.ADS_TYPE_URL_RDS_V2, "")));
  ...
  public void cdsResponseWithoutMatchingResource() {
    ...
    verify(requestObserver)
        .onNext(eq(
            buildDiscoveryRequestV2(NODE, "", "cluster-foo.googleapis.com",
    ...
    DiscoveryResponse response =
        buildDiscoveryResponseV2("0", clusters, EnvoyProtoData.ADS_TYPE_URL_CDS_V2, "0000");
    responseObserver.onNext(response);

    // Client sent an ACK CDS request.
    verify(requestObserver)
        .onNext(eq(
            buildDiscoveryRequestV2(NODE, "0", "cluster-foo.googleapis.com",
                EnvoyProtoData.ADS_TYPE_URL_CDS_V2, "0000")));
  ...
  public void cdsResponseWithMatchingResource() {
    ...
    clusters = ImmutableList.of(
        ...
        Any.pack(buildCluster("cluster-baz.googleapis.com", null, false)));
    response =
        buildDiscoveryResponseV2("1", clusters, EnvoyProtoData.ADS_TYPE_URL_CDS_V2, "0001");
    responseObserver.onNext(response);

    // Client sent an ACK CDS request.
    verify(requestObserver)
        .onNext(eq(
            buildDiscoveryRequestV2(NODE, "1", "cluster-foo.googleapis.com",
                EnvoyProtoData.ADS_TYPE_URL_CDS_V2, "0001")));
  ...
  /**
   * (CDS response caching behavior) Adding cluster watchers interested in some cluster that some
   * other endpoint watcher had already been watching on will result in cluster update notified to
   * the newly added watcher immediately, without sending new CDS requests.
   */
  @Test
  ...
  public void addRemoveClusterWatchers() {
    ...
    verify(requestObserver)
        .onNext(eq(
            buildDiscoveryRequestV2(NODE, "1", "cluster-bar.googleapis.com",
                EnvoyProtoData.ADS_TYPE_URL_CDS_V2, "0001")));
    ...
    response =
        buildDiscoveryResponseV2("2", clusters, EnvoyProtoData.ADS_TYPE_URL_CDS_V2, "0002");
    responseObserver.onNext(response);
        ...
        .onNext(
            argThat(
                new DiscoveryRequestMatcher("2", ImmutableList.<String>of(),
                    EnvoyProtoData.ADS_TYPE_URL_CDS_V2, "0002")));
    ...
    verify(requestObserver)
        .onNext(eq(
            buildDiscoveryRequestV2(NODE, "2", "cluster-foo.googleapis.com",
                EnvoyProtoData.ADS_TYPE_URL_CDS_V2, "0002")));
    ...
    clusters = ImmutableList.of(
        ...
        Any.pack(
            buildCluster("cluster-bar.googleapis.com", null, false)));
    response =
        buildDiscoveryResponseV2("3", clusters, EnvoyProtoData.ADS_TYPE_URL_CDS_V2, "0003");
    responseObserver.onNext(response);
    ...
    verify(requestObserver)
        .onNext(eq(
            buildDiscoveryRequestV2(NODE, "3", "cluster-foo.googleapis.com",
                EnvoyProtoData.ADS_TYPE_URL_CDS_V2, "0003")));
  }
  ...
  public void cdsUpdateForClusterBeingRemoved() {
    ...
    response =
        buildDiscoveryResponseV2("1", ImmutableList.<Any>of(),
            EnvoyProtoData.ADS_TYPE_URL_CDS_V2, "0001");
    responseObserver.onNext(response);
  ...
  /**
   * Client receives an EDS response that does not contain a ClusterLoadAssignment for the requested
   * resource while each received ClusterLoadAssignment is valid. The EDS response is ACKed. After
   * the resource fetch timeout expires, watchers waiting for the resource is notified with resource
   * unavailable.
   */
  @Test
  public void edsResponseWithoutMatchingResource() {
    ...
    DiscoveryResponse response =
        buildDiscoveryResponseV2("0", clusterLoadAssignments,
            EnvoyProtoData.ADS_TYPE_URL_EDS_V2, "0000");
    responseObserver.onNext(response);
  ...
  /**
   * Normal workflow of receiving an EDS response containing ClusterLoadAssignment message for a
   * requested cluster.
   */
  @Test
  public void edsResponseWithMatchingResource() {
    ...
    response =
        buildDiscoveryResponseV2(
            "1", clusterLoadAssignments, EnvoyProtoData.ADS_TYPE_URL_EDS_V2, "0001");
    responseObserver.onNext(response);
    ...
    verify(requestObserver)
        .onNext(eq(
            buildDiscoveryRequestV2(NODE, "1", "cluster-foo.googleapis.com",
                EnvoyProtoData.ADS_TYPE_URL_EDS_V2, "0001")));
  ...
  public void multipleEndpointWatchers() {
    ...
    response = buildDiscoveryResponseV2("1", clusterLoadAssignments,
        EnvoyProtoData.ADS_TYPE_URL_EDS_V2, "0001");
    responseObserver.onNext(response);
   ...
   * has some other endpoint watchers watching on. Endpoint information received previously is in
   * local cache and notified to the new watcher immediately.
   */
  @Test
  ...
  public void addRemoveEndpointWatchers() {
    ...
    response = buildDiscoveryResponseV2("2", clusterLoadAssignments,
        EnvoyProtoData.ADS_TYPE_URL_EDS_V2, "0002");
    responseObserver.onNext(response);
    ...
    verify(requestObserver)
        .onNext(eq(
            buildDiscoveryRequestV2(NODE, "2", "cluster-bar.googleapis.com",
                EnvoyProtoData.ADS_TYPE_URL_EDS_V2, "0002")));
    ...
    response = buildDiscoveryResponseV2("3", clusterLoadAssignments,
        EnvoyProtoData.ADS_TYPE_URL_EDS_V2, "0003");
    responseObserver.onNext(response);
  ...
  /**
   * RPC stream closed and retry during the period of first time resolving service config (LDS/RDS
   * only).
   */
  @Test
  ...
  public void streamClosedAndRetryReschedulesAllResourceFetchTimer() {
    ...
    verify(requestObserver).onNext(
        eq(buildDiscoveryRequestV2(NODE, "", TARGET_AUTHORITY,
            EnvoyProtoData.ADS_TYPE_URL_LDS_V2, "")));
    responseObserver.onNext(response);
                ...
                buildRouteConfiguration(
                    ...
                    ImmutableList.of(
                        buildVirtualHost(
                            ...
                            "cluster.googleapis.com")))));
    DiscoveryResponse response =
        buildDiscoveryResponseV2("213", routeConfigs, EnvoyProtoData.ADS_TYPE_URL_RDS_V2, "0052");
  ...
  public void messagePrinter_printCdsResponse() {
    ...
    List<Any> clusters = ImmutableList.of(
        ...
        Any.pack(buildCluster("cluster-foo.googleapis.com", null, false)));
    DiscoveryResponse response =
        buildDiscoveryResponseV2("14", clusters, EnvoyProtoData.ADS_TYPE_URL_CDS_V2, "8");
  ...
  public void messagePrinter_printEdsResponse() {
    ...
    DiscoveryResponse response =
        buildDiscoveryResponseV2("5", clusterLoadAssignments,
            EnvoyProtoData.ADS_TYPE_URL_EDS_V2, "004");
