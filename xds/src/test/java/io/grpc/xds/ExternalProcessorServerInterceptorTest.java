...
public class ExternalProcessorServerInterceptorTest {
  ...
  private static class LazyInputStreamMarshaller
  ...
  @Test
  public void givenNoOverrideConfig_whenBuildServerInterceptor_thenUsesParentConfig()
      throws Exception {
  ...
  public void givenInterceptor_whenCallIntercepted_thenExtProcStubUsesSynchronizationContext()
    ...
    try {
      ...
      assertThat(capturedExecutor.get())
          .isSameInstanceAs(com.google.common.util.concurrent.MoreExecutors.directExecutor());
    } finally {
  ...
  @Test
  public void givenCallFailed_whenClientHalfClosesAndAppCompletes_thenErrorStatusPreserved()
      throws Exception {
  ...
  @Test
  public void serverInterceptor_clientHalfClose_sendsBothEndOfStreamAndEndOfStreamWithoutMessage()
      throws Exception {
    ExternalProcessor proto =
        createBaseProto(extProcServerName)
            .setProcessingMode(
    ...
    final CountDownLatch extProcLatch = new CountDownLatch(2);
    final java.util.concurrent.atomic.AtomicReference<ProcessingRequest> capturedBodyRequest =
            ...
            return new StreamObserver<ProcessingRequest>() {
              ...
              public void onNext(ProcessingRequest request) {
                if (request.hasRequestHeaders()) {
                  ...
                  capturedBodyRequest.set(request);
                  extProcLatch.countDown();
                }
              }
    ...
    try {
      ...
      assertWithMessage("Ext proc should receive headers and body EOS")
          .that(extProcLatch.await(5, TimeUnit.SECONDS))
          .isTrue();
      ProcessingRequest bodyRequest = capturedBodyRequest.get();
      assertThat(bodyRequest).isNotNull();
      assertThat(bodyRequest.hasRequestBody()).isTrue();
      assertThat(bodyRequest.getRequestBody().getBody().isEmpty()).isTrue();
      assertThat(bodyRequest.getRequestBody().getEndOfStream()).isTrue();
      assertThat(bodyRequest.getRequestBody().getEndOfStreamWithoutMessage()).isTrue();
    } finally {
      clientCall.cancel("Cleanup", null);
      channelManager.close();
    }
  ...
  @Test
  public void serverInterceptor_extProcResponse_eosTrue_eosWithoutMessageTrue_bodyIgnored()
      throws Exception {
    ExternalProcessor proto =
        createBaseProto(extProcServerName)
            .setProcessingMode(
    ...
    try {
      ...
      assertThat(extProcLatch.await(5, TimeUnit.SECONDS)).isTrue();
      // App receives half-close because end_of_stream: true was specified
      assertThat(appHalfCloseLatch.await(5, TimeUnit.SECONDS)).isTrue();
      // "ignored-body" must NOT be delivered because end_of_stream_without_message is true
      assertThat(serverReceivedMessages).isEmpty();
    } finally {
      clientCall.cancel("Cleanup", null);
      channelManager.close();
  ...
  public void
    ...
    try {
      ...
      assertThat(extProcLatch.await(5, TimeUnit.SECONDS)).isTrue();
      assertThat(appHalfCloseLatch.await(5, TimeUnit.SECONDS)).isTrue();
      // Empty body delivered as a regular message
  ...
  @Test
  public void
      serverInterceptor_extProcResponse_eosFalse_eosWithoutMessageTrue_ignoredEosWithoutMessage()
          throws Exception {
    ExternalProcessor proto =
        createBaseProto(extProcServerName)
            .setProcessingMode(
  ...
  public void givenNormalMode_whenClientBusy_thenIsReadyReturnsTrue() throws Exception {
    ...
    try {
      ...
      // Complete ext proc stream
      responseObserverRef.get().onCompleted();

      // Now isReady should delegate to rawCall.isReady() which is false
      assertThat(interceptedCallRef.get().isReady()).isFalse();
    } finally {
      if (responseObserverRef.get() != null) {
        try {
    ...
    dataPlaneHandler =
        new DataPlaneServiceHandler() {
          ...
          public void sayHello(InputStream request, StreamObserver<InputStream> responseObserver) {
            try {
              serverReceivedMessages.add(
                  new String(ByteStreams.toByteArray(request), StandardCharsets.UTF_8));
  ...
  @Test
  public void serverInterceptor_failureModeAllow_failOpen_protocolErrorBeforeBody()
      throws Exception {
    ExternalProcessor proto =
        createBaseProto(extProcServerName)
            .setFailureModeAllow(true)
    ...
    // Trigger protocol error by sending out-of-order response (ResponseBodyResponse instead of
  ...
  @Test
  public void serverInterceptor_failureModeAllow_deferredHalfClose_failOpen() throws Exception {
    ExternalProcessor proto =
        createBaseProto(extProcServerName)
            .setFailureModeAllow(true)
            .setProcessingMode(
    ...
    // We need to capture half-close on the server app listener.
    ServerInterceptor capturingInterceptor =
        new ServerInterceptor() {
          @Override
          public <ReqT, RespT> ServerCall.Listener<ReqT> interceptCall(
    ...
    final List<ByteString> clientResponseMessages = new ArrayList<>();
    final AtomicReference<Status> closedStatus = new AtomicReference<>();
    clientCall.start(
        new io.grpc.ClientCall.Listener<InputStream>() {
          @Override
    ...
    // Client half-closes immediately (should be deferred because we are IDLE - waiting for
    // headers response)
    clientCall.halfClose();

    // Verify half-close NOT delivered to app yet
    assertThat(appHalfCloseLatch.await(1, TimeUnit.SECONDS)).isFalse();

    // Now trigger stream failure (onError) BEFORE headers response -> should trigger fail-open
    extProcResponseObserverRef.get().onError(
        Status.UNAVAILABLE.withDescription("Simulated sidecar failure").asRuntimeException());

    // Verify fail-open:
    // 1. Deferred half-close should be delivered to app (triggering onCompleted in our handler)
    // 2. Call should complete successfully
  ...
  public void serverInterceptor_clientRequest_bufferFailure_idle() throws Exception {
    ...
    // Respond to headers to activate call.
    // This will trigger rawNext.startCall -> request(2) -> onMessage (synchronously) -> IOException
    // -> close.
    extProcResponseObserverRef
        .get()
        .onNext(
            ProcessingResponse.newBuilder()
                .setRequestHeaders(
  ...
  public void serverInterceptor_observabilityMode_responseBody_defaultMode() throws Exception {
    ...
    dataPlaneHandler =
        new DataPlaneServiceHandler() {
          ...
          public void sayHello(InputStream request, StreamObserver<InputStream> responseObserver) {
            responseObserver.onNext(
                new ByteArrayInputStream("response_message".getBytes(StandardCharsets.UTF_8)));
            responseObserver.onCompleted();
          }
        };
  ...
  @Test
  public void serverInterceptor_observabilityMode_serverCloseBeforeClientHalfClose()
      throws Exception {
    ExternalProcessor proto =
        createBaseProto(extProcServerName)
            .setObservabilityMode(true)
  ...
  @Test
  public void serverInterceptor_observabilityMode_trailersOnly_sendHeaders() throws Exception {
    final String uniqueExtProcServerName = InProcessServerBuilder.generateName();
    ExternalProcessor proto =
        createBaseProto(uniqueExtProcServerName)
            .setObservabilityMode(true)
    ...
    // Use custom service with raw ServerCallHandler to send custom trailers
    ServerServiceDefinition customService = ServerServiceDefinition.builder("test.TestService")
        .addMethod(
            METHOD_SAY_HELLO_BIDI,
            new ServerCallHandler<InputStream, InputStream>() {
              @Override
              public ServerCall.Listener<InputStream> startCall(
                ...
                Metadata trailers = new Metadata();
                trailers.put(
                    Metadata.Key.of("x-custom-trailer", Metadata.ASCII_STRING_MARSHALLER),
                    "trailer-val");
                call.close(Status.OK, trailers); // Triggers trailers-only
                return new ServerCall.Listener<InputStream>() {};
              }
            })
        .build();
    ...
    grpcCleanup.register(
        InProcessServerBuilder.forName(dataPlaneServerName)
            .addService(ServerInterceptors.intercept(customService, interceptor))
            .directExecutor()
            .build()
            .start());
    dataPlaneChannel =
    ...
    io.envoyproxy.envoy.config.core.v3.HeaderValue customTrailer = null;
    for (io.envoyproxy.envoy.config.core.v3.HeaderValue hv : headerMap.getHeadersList()) {
      if (hv.getKey().equals("x-custom-trailer")) {
        customTrailer = hv;
        break;
      }
    }
    assertThat(customTrailer).isNotNull();
    assertThat(customTrailer.getRawValue().toStringUtf8()).isEqualTo("trailer-val");
  ...
  public void serverInterceptor_immediateResponse_duringResponseHeaders_closesCall()
    ...
    assertThat(closedStatus.get().getCode()).isEqualTo(Status.Code.UNAUTHENTICATED);
    assertThat(closedStatus.get().getDescription())
        .isEqualTo("Immediate Auth Failure on Response Headers");
        ...
        new ExternalProcessorGrpc.ExternalProcessorImplBase() {
          ...
          public StreamObserver<ProcessingRequest> process(
            ...
            return new StreamObserver<ProcessingRequest>() {
              ...
              public void onNext(ProcessingRequest request) {
                if (request.hasRequestHeaders()) {
                  // Violate order: send ResponseTrailers response when RequestHeadersResponse is
                  // expected
                  responseObserver.onNext(
                      ProcessingResponse.newBuilder()
                          .setResponseTrailers(TrailersResponse.newBuilder().build())
  ...
  public void testFlowControlStateInitialization() throws Exception {
            ...
            return new StreamObserver<ProcessingRequest>() {
              ...
              public void onNext(ProcessingRequest request) {
                ...
                if (request.hasRequestHeaders()) {
                  ...
                  boolean isEosWithoutMsg =
                      request.getRequestBody().getEndOfStreamWithoutMessage();
                  if (isEos && (isEosWithoutMsg || request.getRequestBody().getBody().isEmpty())) {
                    responseObserver.onNext(
                        ProcessingResponse.newBuilder()
                            .setRequestBody(
                                BodyResponse.newBuilder()
    ...
    assertThat(thirdRequest.getRequestBody().getEndOfStream()).isTrue();
  ...
  public void testDownstreamToSidestreamFlowControl_EnforcesWindow() throws Exception {
    ...
    final List<ByteString> dataPlaneReceivedMessages =
        new java.util.concurrent.CopyOnWriteArrayList<>();
    final CountDownLatch serverAppCompletedLatch = new CountDownLatch(1);
    ...
    final List<ByteString> clientResponseMessages =
        new java.util.concurrent.CopyOnWriteArrayList<>();
    final CountDownLatch callCompletedLatch = new CountDownLatch(1);
  ...
  public void serverInterceptor_deferredClose_pendingResponseMessages() throws Exception {
                            ...
                            .setBodyMutation(
                                BodyMutation.newBuilder()
                                    .setStreamedResponse(
                                        StreamedBodyResponse.newBuilder()
                                            .setBody(
                                                receivedRequests.get(0).getResponseBody().getBody())
                                            .build())
                                    .build())
                            .build())
                    .build())
                            ...
                            .setBodyMutation(
                                BodyMutation.newBuilder()
                                    .setStreamedResponse(
                                        StreamedBodyResponse.newBuilder()
                                            .setBody(
                                                receivedRequests.get(1).getResponseBody().getBody())
                                            .build())
                                    .build())
                            .build())
                    .build())
    ...
    assertThat(receivedRequests.get(0).getResponseBody().getBody())
        .isEqualTo(ByteString.copyFrom(largeMessageBytes));
    assertThat(receivedRequests.get(1).hasResponseBody()).isTrue();
    assertThat(receivedRequests.get(1).getResponseBody().getBody())
        .isEqualTo(ByteString.copyFrom(smallMessageBytes));
    assertThat(receivedRequests.get(2).hasClientWindowUpdate()).isTrue();
  ...
  public void testDownstreamToSidestreamFlowControl_deferredHalfClose() throws Exception {
    ...
    assertThat(appHalfCloseLatch.await(500, TimeUnit.MILLISECONDS)).isFalse();
    // Verify server app didn't receive any messages yet (Msg 1 is still in ext_proc processing, Msg
    // 2 is buffered)
    assertThat(serverReceivedMessages).isEmpty();
    ...
    assertThat(appHalfCloseLatch.await(5, TimeUnit.SECONDS)).isTrue();
    assertThat(serverReceivedMessages)
        .containsExactly(new String(largeMessageBytes, StandardCharsets.UTF_8), "b");
  ...
  public void testFailOpen_DrainsInboundQueuesInOrder() throws Exception {
    ...
    synchronized (dataPlaneCall.streamLock) {
      dataPlaneCall.pendingMutatedRequestBodies.add(
          StreamedBodyResponse.newBuilder().setBody(ByteString.copyFromUtf8("mutated-1")).build());
      dataPlaneCall.pendingRequestBodyMessages.add(ByteString.copyFromUtf8("raw-waiting-2"));
      listener.savedMessages.add(
          new ByteArrayInputStream("raw-saved-3".getBytes(StandardCharsets.UTF_8)));
    }
  ...
  public void testUpstreamToSidestreamFlowControl_EnforcesWindow() throws Exception {
    ...
    final List<ByteString> clientReceivedMessages =
        new java.util.concurrent.CopyOnWriteArrayList<>();
  ...
  public void testClientWindowUpdateSentImmediatelyOnSidestreamToDownstreamWindowExhaustion()
    ...
    final List<ProcessingRequest> receivedRequests =
        new java.util.concurrent.CopyOnWriteArrayList<>();
    final CountDownLatch sidecarLatch =
        new CountDownLatch(4); // Response Headers + Response Body 1 + 2 + 3
  ...
  public void testThresholdBasedWindowUpdates() throws Exception {
    ...
    final List<ProcessingRequest> receivedRequests =
        new java.util.concurrent.CopyOnWriteArrayList<>();
    final CountDownLatch sidecarLatch =
        new CountDownLatch(3); // Response Headers + Response Body 1 + 2
  ...
  public void testFailOpenDrainsResponseQueues() throws Exception {
    ...
    final CountDownLatch sidecarLatch =
        new CountDownLatch(3); // Response Headers + Body 1 (50k) + Body 2 (20k)
    final AtomicReference<StreamObserver<ProcessingResponse>> responseObserverRef =
        new AtomicReference<>();
    ExternalProcessorGrpc.ExternalProcessorImplBase extProcImpl =
        new ExternalProcessorGrpc.ExternalProcessorImplBase() {
        ...
        new DataPlaneServiceHandler() {
          ...
          public StreamObserver<InputStream> sayHelloBidi(
            ...
            return new StreamObserver<InputStream>() {
              ...
              public void onNext(InputStream value) {
                responseObserver.onNext(
                    new ByteArrayInputStream("resp1".getBytes(StandardCharsets.UTF_8)));
                responseObserver.onNext(
                    new ByteArrayInputStream("resp2".getBytes(StandardCharsets.UTF_8)));
              }
  ...
  public void testAppPullModel() throws Exception {
    ...
    final AtomicReference<ServerCallStreamObserver<InputStream>> serverCallObserverRef =
        new AtomicReference<>();
  ...
  public void givenExtProcCall_whenExecutionSucceeds_thenAll4MetricsAreRecorded() throws Exception {
    ...
    final CountDownLatch extProcLatch =
        new CountDownLatch(5); // Req Headers, Req Body, Req EOS, Resp Headers, Resp Trailers
        ...
        new DataPlaneServiceHandler() {
          ...
          public StreamObserver<InputStream> sayHelloBidi(
            ...
            return new StreamObserver<InputStream>() {
              ...
              public void onNext(InputStream value) {
                responseObserver.onNext(
                    new ByteArrayInputStream("resp".getBytes(StandardCharsets.UTF_8)));
              }
    ...
    Mockito.verify(mockMetricRecorder, Mockito.times(1))
  ...
  @Test
  public void serverInterceptor_messageAfterStreamCompleted_failOpen() throws Exception {
    ExternalProcessor proto =
        createBaseProto(extProcServerName)
            .setFailureModeAllow(true)
            .setProcessingMode(
    ...
    // Now trigger stream failure (onError) -> should trigger fail-open
    extProcResponseObserverRef.get().onError(
        Status.UNAVAILABLE.withDescription("Simulated sidecar failure").asRuntimeException());

    // Wait a bit to ensure fail-open is processed
    Thread.sleep(100);

    // Send client message AFTER stream is completed (failed)
    clientCall.sendMessage(new ByteArrayInputStream("msg1".getBytes(StandardCharsets.UTF_8)));
    ...
    // Verify:
    // 1. Message should be delivered to server app directly (bypassing ext proc)
    // 2. Call should complete successfully
    assertThat(serverAppCompletedLatch.await(5, TimeUnit.SECONDS)).isTrue();
    assertThat(serverReceivedMessages).containsExactly("msg1");

    channelManager.close();
  }

  @Test
  public void serverInterceptor_operationsAfterClose() throws Exception {
    ExternalProcessor proto =
        createBaseProto(extProcServerName)
            .setFailureModeAllow(true)
            .build();
    ...
    final AtomicReference<ServerCall<InputStream, InputStream>> capturedCall =
        new AtomicReference<>();
    ServerInterceptor capturingInterceptor =
        new ServerInterceptor() {
          @Override
          ...
          public <ReqT, RespT> ServerCall.Listener<ReqT> interceptCall(
              ServerCall<ReqT, RespT> call, Metadata headers, ServerCallHandler<ReqT, RespT> next) {
            capturedCall.set((ServerCall<InputStream, InputStream>) call);
            return next.startCall(call, headers);
          }
        };
    ...
    assertThat(capturedCall.get()).isNotNull();

    // 1. Test sendMessage after close: should return early and not throw
    try {
      capturedCall
          .get()
          .sendMessage(new ByteArrayInputStream("extra".getBytes(StandardCharsets.UTF_8)));
    } catch (Exception e) {
      org.junit.Assert.fail("Should not throw when sendMessage is called after close: " + e);
    }

    // 2. Test double close: should return early and not throw
    try {
      capturedCall.get().close(Status.OK, new Metadata());
    } catch (Exception e) {
      org.junit.Assert.fail("Should not throw when close is called again: " + e);
    }

    // 3. Test isReady after close: should return false and not throw
    try {
      assertThat(capturedCall.get().isReady()).isFalse();
    } catch (Exception e) {
