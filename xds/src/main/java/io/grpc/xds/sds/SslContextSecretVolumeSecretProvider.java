...
/**
 * An SslContext provider that uses file-based secrets (secret volume).
 * Used for both server and client SslContexts
 */
final class SslContextSecretVolumeSecretProvider
    implements SecretProvider<SslContext> {
  @VisibleForTesting
  ...
  SslContext buildSslContextFromSecrets() throws SSLException {
    ...
    if (server) {
      sslContextBuilder = GrpcSslContexts.forServer(new File(certificateChain),
              new File(privateKey), privateKeyPassword);
      if (trustedCa != null) {
        sslContextBuilder.trustManager(new File(trustedCa));
      }
    } else {
      sslContextBuilder = GrpcSslContexts.forClient()
              .trustManager(new File(trustedCa));
      if (privateKey != null && certificateChain != null) {
        sslContextBuilder.keyManager(new File(certificateChain),
                new File(privateKey), privateKeyPassword);
      }
    }
