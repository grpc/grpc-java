...
final class XdsServerWrapper extends Server {
  ...
  private final class DiscoveryState implements ResourceWatcher<LdsUpdate> {
    ...
    private void updateSelector() {
      ...
      // TODO(sergiitk): [design] implements filter chain replacement logic
    ...
    private final class RouteDiscoveryState implements ResourceWatcher<RdsUpdate> {
      ...
      private void maybeUpdateSelector() {
        isPending = false;
        // TODO(sergiitk): [design] ensures RDS does not cycle connections (see updateSelector);
  ...
  @VisibleForTesting
  // TODO(sergiitk): for the life of the server
  final class ConfigApplyingInterceptor implements ServerInterceptor {
    ...
    public <ReqT, RespT> Listener<ReqT> interceptCall(ServerCall<ReqT, RespT> call,
        Metadata headers, ServerCallHandler<ReqT, RespT> next) {
      // TODO(sergiitk): [design] internal server interceptor: RPC matching logic.
  ...
  abstract static class ServerRoutingConfig {
    ...
    // TODO(sergiitk): [design] contains per-route interceptor generated by buildserverinterceptopr
