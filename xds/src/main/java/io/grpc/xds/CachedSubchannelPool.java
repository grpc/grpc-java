...
final class CachedSubchannelPool implements SubchannelPool {
  ...
  private final HashMap<List<EquivalentAddressGroup>, CacheEntry> cache = new HashMap<>();
  ...
  public Subchannel takeOrCreateSubchannel(
      List<EquivalentAddressGroup> eags, Attributes defaultAttributes) {
    final CacheEntry entry = cache.remove(eags);
    final Subchannel subchannel;
    if (entry == null) {
      subchannel = helper.createSubchannel(eags, defaultAttributes);
    } else {
  ...
  public void handleSubchannelState(Subchannel subchannel, ConnectivityStateInfo newStateInfo) {
    CacheEntry cached = cache.get(subchannel.getAllAddresses());
  ...
  public void returnSubchannel(Subchannel subchannel, ConnectivityStateInfo lastKnownState) {
    CacheEntry prev = cache.get(subchannel.getAllAddresses());
    if (prev != null) {
    ...
    cache.put(subchannel.getAllAddresses(), entry);
  }
