...
```gradle
compile 'io.grpc:grpc-all:0.14.1-SNAPSHOT'
```

For Android client, you only need to depend on the needed sub-projects, such as:
```gradle
compile 'io.grpc:grpc-okhttp:0.14.1-SNAPSHOT'
compile 'io.grpc:grpc-protobuf-nano:0.14.1-SNAPSHOT'
compile 'io.grpc:grpc-stub:0.14.1-SNAPSHOT'
```

[the JAR]: https://search.maven.org/remote_content?g=io.grpc&a=grpc-all&v=0.14.1-SNAPSHOT
  ...
  <plugins>
    <plugin>
      ...
      <configuration>
        ...
        <pluginArtifact>io.grpc:protoc-gen-grpc-java:0.14.1-SNAPSHOT:exe:${os.detected.classifier}</pluginArtifact>
      </configuration>
      <executions>
        <execution>
          <goals>
...
protobuf {
  ...
  plugins {
    grpc {
      artifact = 'io.grpc:protoc-gen-grpc-java:0.14.1-SNAPSHOT'
    }
  }
  generateProtoTasks {
    all()*.plugins {
