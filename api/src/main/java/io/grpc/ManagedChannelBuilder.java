...
public abstract class ManagedChannelBuilder<T extends ManagedChannelBuilder<T>> {
   ...
   * </ul>
   *
   * <p>We strongly recommend the URI form of {@code target} because the alternative is ambiguous.
   * For example, the target string {@code foo:8080} is a valid authority string with host
   * {@code foo} and port {@code 8080} but it is also a valid RFC 3986 URI with scheme {@code foo}
   * and path {@code 8080}. {@code NameResolver}s are discovered dynamically from your classpath
   * using SPI, so it's hard to be sure in advance how such a target will be resolved. A
   * {@code NameResolver} for scheme {@code foo} might someday make a host named {@code foo}
   * unreachable! On the other hand, the {@code dns:///foo:8080} target will always behave the same.
   *
   * <p>Note that there is an open JDK bug on {@link java.net.URI} class parsing an ipv6 scope ID:
   * bugs.openjdk.org/browse/JDK-8199396. This method is exposed to this bug. If you experience an
   * issue, a work-around is to convert the scope ID to its numeric form (e.g. by using
