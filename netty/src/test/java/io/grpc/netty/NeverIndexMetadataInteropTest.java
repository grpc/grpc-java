...
public class NeverIndexMetadataInteropTest {
  ...
  private static final Metadata.Key<String> NEVER_INDEXED_RESPONSE_METADATA_KEY =
      Metadata.Key.of("x-hpack-never-indexed-response", Metadata.ASCII_STRING_MARSHALLER);
  ...
  @Test
  public void neverIndexedResponseMetadataInteroperates() throws Exception {
    String neverIndexedValue = "response-metadata-value";
    Metadata responseMetadata = new Metadata();
    responseMetadata.put(NEVER_INDEXED_RESPONSE_METADATA_KEY, neverIndexedValue);
    server = NettyServerBuilder.forPort(0)
        .neverIndexMetadataKey(NEVER_INDEXED_RESPONSE_METADATA_KEY)
        .addService(
            ServerInterceptors.intercept(
                new SimpleServiceImpl(),
    ...
    SimpleResponse response = stub.withDeadlineAfter(10, TimeUnit.SECONDS)
        .unaryRpc(SimpleRequest.getDefaultInstance());
    ...
    assertEquals(
        neverIndexedValue,
        headersCapture.get().get(NEVER_INDEXED_RESPONSE_METADATA_KEY));
  }
