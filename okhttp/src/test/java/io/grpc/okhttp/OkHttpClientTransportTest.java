...
public class OkHttpClientTransportTest {
  ...
  @Test
  public void goAway_unrecognizedErrorCode() throws Exception {
    initTransport();
    // Simulate GOAWAY with an unrecognized error code (represented as null ErrorCode).
    frameHandler().goAway(0, null, ByteString.encodeUtf8("apache error 70007"));

    ArgumentCaptor<Status> statusCaptor = ArgumentCaptor.forClass(Status.class);
    ArgumentCaptor<DisconnectError> disconnectErrorCaptor =
        ArgumentCaptor.forClass(DisconnectError.class);
    verify(transportListener, timeout(TIME_OUT_MS)).transportShutdown(statusCaptor.capture(),
        disconnectErrorCaptor.capture());

    Status status = statusCaptor.getValue();
    assertEquals(Status.INTERNAL.getCode(), status.getCode());
    assertTrue(status.getDescription(),
        status.getDescription().contains("Unrecognized HTTP/2 error code"));
    assertTrue(status.getDescription(),
        status.getDescription().contains("apache error 70007"));
    ...
    assertEquals(new GoAwayDisconnectError(GrpcUtil.Http2Error.INTERNAL_ERROR), disconnectError);

    shutdownAndVerify();
  }

  @Test
  public void goAway_unrecognizedErrorCode_streamListenerRpcProgress() throws Exception {
    initTransport();
    setMaxConcurrentStreams(2);
    MockStreamListener listener1 = new MockStreamListener();
    MockStreamListener listener2 = new MockStreamListener();
